...
| S12 | Does a cluster store user-defined callout rules? | **Open (cluster, 2026-09-10).** On the dev cluster it does not. `.alter cluster policy callout` succeeds, and `.show cluster policy callout` keeps returning exactly the same 21 rules, all built-in and all with `CanCall: true` -- also for the single rule copied verbatim from Kusto's documentation (`sqlname\.database\.azure\.com/?$`). The resource therefore reported Synced while the rule was never stored, and drifted on every poll. Two questions are open: whether the command has to be `.alter-merge` (as it does for capacity), and whether this cluster or SKU refuses user callouts at all. Settle it by hand: run the alter, then the show, and see whether a 22nd rule appears. `calloutpolicy.yaml` is excluded from e2e until then. **Postponed past v0.1.0 (2026-09-10):** the kind ships, with the limitation stated in the README and in the package description, rather than holding the release for it. | CalloutPolicy |
